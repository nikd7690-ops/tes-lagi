-- [[ ALPHA PROJECT - GOD MODE AUTO FARM & COLLECT (SMART RADAR VERSION) ]] --
...
-- Ambil WorldManager untuk Radar Pintar
...
StartFrame.BackgroundColor3 = Theme.Item 
Instance.new("UICorner", StartFrame).CornerRadius = UDim.new(0, 6)
StartFrame.ZIndex = 1

local StartLbl = Instance.new("TextLabel", StartFrame)
StartLbl.Size = UDim2.new(0.6, 0, 1, 0); StartLbl.Position = UDim2.new(0, 10, 0, 0); StartLbl.Text = "Auto Farm"; StartLbl.TextColor3 = Theme.Text; StartLbl.Font = Enum.Font.Gotham; StartLbl.TextSize = 12; StartLbl.BackgroundTransparency = 1; StartLbl.TextXAlignment = Enum.TextXAlignment.Left

local StartBtn = Instance.new("TextButton", StartFrame)
StartBtn.Size = UDim2.new(0.45, -10, 0.1, 22); StartBtn.Position = UDim2.new(0.58, -10, 0.5, -11); StartBtn.BackgroundColor3 = Theme.Main; StartBtn.Text = "OFF"; StartBtn.TextColor3 = Color3.fromRGB(255, 80, 80); StartBtn.Font = Enum.Font.GothamBold; StartBtn.TextSize = 10; Instance.new("UICorner", StartBtn).CornerRadius = UDim.new(0, 4)
local StartStroke = Instance.new("UIStroke", StartBtn); StartStroke.Color = Color3.fromRGB(255, 80, 80); StartStroke.Thickness = 1

StartBtn.MouseButton1Click:Connect(function()
    _G.Farm_Active = not _G.Farm_Active
    if _G.Farm_Active then
        StartBtn.Text = "ON"; StartBtn.TextColor3 = Theme.Accent; StartStroke.Color = Theme.Accent
        TS:Create(StartBtn, TweenInfo.new(0.2), {BackgroundColor3 = Theme.Main}):Play()
    else
        StartBtn.Text = "OFF"; StartBtn.TextColor3 = Color3.fromRGB(255, 80, 80); StartStroke.Color = Color3.fromRGB(255, 80, 80)
        TS:Create(StartBtn, TweenInfo.new(0.2), {BackgroundColor3 = Theme.Main}):Play()
    end
end)
...
PosFrame.Size = UDim2.new(1, -10, 0, 35); PosFrame.BackgroundColor3 = Theme.Item; Instance.new("UICorner", PosFrame).CornerRadius = UDim.new(0, 6); PosFrame.ZIndex = 1

local PosLbl = Instance.new("TextLabel", PosFrame)
PosLbl.Size = UDim2.new(0.4, 0, 1, 0); PosLbl.Position = UDim2.new(0, 10, 0, 0); PosLbl.Text = "Save Position"; PosLbl.TextColor3 = Theme.Text; PosLbl.Font = Enum.Font.Gotham; PosLbl.TextSize = 12; PosLbl.BackgroundTransparency = 1; PosLbl.TextXAlignment = Enum.TextXAlignment.Left

local CoordLbl = Instance.new("TextLabel", PosFrame)
CoordLbl.Size = UDim2.new(0.3, 0, 1, 0); CoordLbl.Position = UDim2.new(0.4, 0, 0, 0); CoordLbl.Text = "[ NONE ]"; CoordLbl.TextColor3 = Theme.SubText; CoordLbl.Font = Enum.Font.Gotham; CoordLbl.TextSize = 10; CoordLbl.BackgroundTransparency = 1; CoordLbl.TextXAlignment = Enum.TextXAlignment.Center

local SaveBtn = Instance.new("TextButton", PosFrame)
SaveBtn.Size = UDim2.new(0.25, -10, 0.1, 22); SaveBtn.Position = UDim2.new(0.75, 0, 0.5, -11); SaveBtn.BackgroundColor3 = Theme.Main; SaveBtn.Text = "SAVE"; SaveBtn.TextColor3 = Theme.Accent; SaveBtn.Font = Enum.Font.GothamBold; SaveBtn.TextSize = 10; Instance.new("UICorner", SaveBtn).CornerRadius = UDim.new(0, 4)
local SaveStroke = Instance.new("UIStroke", SaveBtn); SaveStroke.Color = Theme.Accent; SaveStroke.Thickness = 1
...
local DropLbl = Instance.new("TextLabel", DropRow); DropLbl.Size = UDim2.new(0.5, 0, 1, 0); DropLbl.Position = UDim2.new(0, 10, 0, 0); DropLbl.Text = "Target Farm Block"; DropLbl.TextColor3 = Theme.Text; DropLbl.Font = Enum.Font.Gotham; DropLbl.TextSize = 12; DropLbl.BackgroundTransparency = 1; DropLbl.TextXAlignment = Enum.TextXAlignment.Left
local DropBtn = Instance.new("TextButton", DropRow); DropBtn.Size = UDim2.new(0.45, -10, 0.8, 0); DropBtn.Position = UDim2.new(0.55, 0, 0.1, 0); DropBtn.BackgroundColor3 = Theme.Main; DropBtn.Text = "Select Block..."; DropBtn.TextColor3 = Theme.SubText; DropBtn.Font = Enum.Font.Gotham; DropBtn.TextSize = 11; Instance.new("UICorner", DropBtn).CornerRadius = UDim.new(0, 6)
local DropList = Instance.new("ScrollingFrame", DropRow); DropList.Size = UDim2.new(0.45, -10, 0, 120); DropList.Position = UDim2.new(0.55, 0, 1.1, 0); DropList.BackgroundColor3 = Theme.Main; DropList.Visible = false; DropList.BorderSizePixel = 0; DropList.ScrollBarThickness = 2; DropList.ZIndex = 100; Instance.new("UICorner", DropList).CornerRadius = UDim.new(0, 6)
...
local function RefreshDropdown()
    ...
    for displayName, itemData in pairs(GetInventoryItems()) do
        local ItemBtn = Instance.new("TextButton", DropList); ItemBtn.Size = UDim2.new(1, 0, 0, 25); ItemBtn.BackgroundTransparency = 1; ItemBtn.Text = displayName; ItemBtn.TextColor3 = Theme.SubText; ItemBtn.Font = Enum.Font.Gotham; ItemBtn.TextSize = 11; ItemBtn.ZIndex = 101
...
local function CreateSetting(label, defaultVal, globalVar)
    local Frame = Instance.new("Frame", Page); Frame.Size = UDim2.new(1, -10, 0, 35); Frame.BackgroundColor3 = Theme.Item; Instance.new("UICorner", Frame).CornerRadius = UDim.new(0, 6); Frame.ZIndex = 1
...
local CollectFrame = Instance.new("Frame", Page); CollectFrame.Size = UDim2.new(1, -10, 0, 35); CollectFrame.BackgroundColor3 = Theme.Item; Instance.new("UICorner", CollectFrame).CornerRadius = UDim.new(0, 6); CollectFrame.ZIndex = 1
...
-- [[ 4. GRID SELECTOR ]] --
local GridBox = Instance.new("Frame", Page); GridBox.Size = UDim2.new(1, -10, 0, 180); GridBox.BackgroundColor3 = Theme.Item; Instance.new("UICorner", GridBox).CornerRadius = UDim.new(0, 6); GridBox.ZIndex = 1
local GInner = Instance.new("Frame", GridBox); GInner.Size = UDim2.new(0, 160, 0, 160); GInner.Position = UDim2.new(0.5, -80, 0.5, -80); GInner.BackgroundTransparency = 1
...
-- [UPGRADE] Fungsi Vacuum Kolektor yang jauh lebih agresif dan stabil
local function StealthCollectDrops()
    ...
    -- MENGULANG 3 KALI: Memastikan tidak ada drop yang telat spawn/tersisa
    for attempt = 1, 3 do
        local hasCollected = false
        
        for _, item in ipairs(Drops:GetChildren()) do
            if not _G.Farm_Active or not _G.AutoCollect then return end
            ...
            -- Pastikan item masih ada di map
            if targetPart and targetPart.Parent then
                ...
                if validGrids[itemX .. "," .. itemY] then
                    ...
                    -- Spam sentuhan 3 kali agar 100% masuk tas
                    ...
                    task.wait(0.15) -- Jeda biar barang ditarik server
                end
            end
        end
        
        if not hasCollected then break end -- Area sudah bersih, tidak perlu diulang lagi
        task.wait(0.2)
    end
...
local function CheckAndSwitchSlot()
    ...
    pcall(function()
        ...
        if currentData and currentData.Id == _G.Farm_ItemID and (currentData.Amount and currentData.Amount > 0) then return end
...
task.spawn(function()
    while true do
        if _G.Farm_Active and #_G.Farm_Targets > 0 then
            ...
            -- [UPGRADE 1: PLACE SMART] Hanya taruh barang kalau areanya kosong!
            for _, o in ipairs(_G.Farm_Targets) do
                if not _G.Farm_Active then break end
                local tx, ty = math.floor(cp.X + o.X), math.floor(cp.Y + o.Y)
                
                local tile = WorldManager.GetTile(tx, ty, 1)
                -- Kalau kosong atau isinya background, baru kita place
                if not tile or tile == 0 or string.find(GetTileName(tile), "background") then
            ...
            task.wait(0.25) -- Jeda nafas agar server merender blok yang baru ditaruh
            
            -- [UPGRADE 2: HIT SMART] Hanya pukul area yang BENAR-BENAR ADA BLOCK-NYA!
            for i = 1, _G.Farm_HitCount do
                ...
                for _, o in ipairs(_G.Farm_Targets) do
                    ...
                    if tile and tile ~= 0 then
                        local tName = GetTileName(tile)
                        if not string.find(tName, "background") and not string.find(tName, "bg") then
                            -- Blok terdeteksi, HANCURKAN!
